\begin{abstract}
    ...
    query language as well as its features are introduced. The code a JIT would
    have generated for several heavy queries is handwritten and benchmarked
    against the same expressions evaluated with the currently employed tree
    walk interpreter. The paper explores the different possibilities for
    accessing the Go compiler, working with the Go plugin API and highlights
    several benchmarks comparing the performance of the new JIT compiler and
    the previous language evaluation implementation.
